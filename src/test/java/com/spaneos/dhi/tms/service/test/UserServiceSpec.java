...
public class UserServiceSpec {
	...
	@Test
	@DisplayName("Registring multiple users")
    void registerUsers() {
		Path path = getExcelFilePath();
		System.out.println("File Path:"+path.toString());
		List<UserDTO> users = loadUserFactory.getLoadUserService(fileTypeDecider, path).loadUsers();
	...
	void findAllWithPaginationWithSort() {
		...
		Page<UserDTO> retObject = userService.findAll(page);
		List<String> pageUserNames = retObject.getContent().stream().map(UserDTO::getUsername)
	...
	private Path getExcelFilePath() {
		return Paths.get(new ClassPathResource("users.xlsx").getPath());
	}

	private Path getCsvFilePath() {
		return Paths.get(new ClassPathResource("users.csv").getPath());
	}
